...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            ipos = distribution.argMax()
            idis = self.distancer.getDistance(pacmanPosition, ipos)
            if idis < fdis:
                fdis = idis
                fpos = ipos
        ...
        idis = 9999999
        for i in legal:
            succ_pos = Actions.getSuccessor(pacmanPosition, i)
            fdis = self.distancer.getDistance(succ_pos, fpos)
            if fdis < idis:
                idis = fdis
                chosenaction = i
        return chosenaction
